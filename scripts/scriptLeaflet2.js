        ...
        for (i = 0; i < feature.geometry.coordinates.length; i++) {
          for (j = 0; j < feature.geometry.coordinates[i].length; j++) {
            if (j % 50 == 0) {
              ...
              ).bindTooltip(feature.properties.value.toString(), {
                permanent: true,
                direction: 'center',
                className: 'my-labels'
              });
...
function show_pga2() {
  ...
  function plot_pga(pga) {
    ...
    layers_combined.addTo(map2);
    control2.addBaseLayer(layers_combined, 'PGA');
  }
}
...
function show_pgv2() {
  ...
  function plot_pgv(pgv) {
    ...
    control2.addBaseLayer(layers_combined, 'PGV');
  }
}
...
function show_psa0p32() {
  ...
  function plot_psa0p3(psa0p3) {
    ...
    control2.addBaseLayer(layers_combined, 'PSA 0.3');
  }
}
...
function show_psa1p02() {
  ...
  function plot_psa1p0(psa1p0) {
    ...
    control2.addBaseLayer(layers_combined, 'PSA 1.0 s');
  }
}
